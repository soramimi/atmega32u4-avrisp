...
bool enter_bitbang_mode(Connection *conn)
{
	...
	int n = conn->read(buf, sizeof(buf), 10);
	if (n == expect.size() && memcmp(buf, expect.data(), expect.size()) == 0) {
		return true;
	}
	return false;
}
...
{
	enter_bitbang_mode(conn);
	
	for (int i = 0; i < 10; i++) {
		write_pin(conn, PIN_RST, true);
		msleep(500);
		write_pin(conn, PIN_RST, false);
		msleep(500);
	}	
}
